...
bool SecFimc::create(int fimc_dev, int fimc_mode, unsigned int buf_num)
{
    ...
    ALOGD("SecFimc::create(fimc_dev: %d, fimc_mode: %d, buf_num: %d)\n", fimc_dev, fimc_mode, buf_num);

    /*
    if(fimc_mode == FIMC_OVLY_DMA_AUTO)
        ALOGD("############# SecFimc::create(FIMC_OVLY_DMA_AUTO) called \n");
    else if(fimc_mode == FIMC_OVLY_NONE_SINGLE_BUF)
        ALOGD("############# SecFimc::create(FIMC_OVLY_NONE_SINGLE_BUF) called \n");
    else
        ALOGD("############# SecFimc::create(%x) called \n", fimc_mode);
    */
    char node[20];
    struct v4l2_format fmt;
    struct v4l2_control     vc;
    ...
    if(mS5pFimc.dev_fd == 0) {
        ...
        if(mS5pFimc.dev_fd < 0) {
            ALOGE("%s:: %s open error\n", __func__, node);
            mS5pFimc.dev_fd = 0;
            return false;
        } else
            ALOGD("fimc%d fd = %d", fimc_dev, (int)mS5pFimc.dev_fd);
    }
#ifdef DEBUG_LIB_FIMC
    ...
    if (ret < 0) {
        ALOGE("%s::VIDIOC_QUERYCAP failed\n", __func__);
        return false;
    }

    if (!(mFimcCap.capabilities & V4L2_CAP_STREAMING)) {
        ALOGE("%s::%s has no streaming support\n", __func__, node);
        return false;
    }

    if (!(mFimcCap.capabilities & V4L2_CAP_VIDEO_OUTPUT)) {
        ALOGE("%s::%s is no video output\n", __func__, node);
        return false;
    }
...
#ifdef DEBUG_LIB_FIMC
    ALOGD("%s:: Fimc reserved memory =0x%8x\n", __func__, mFimcRrvedPhysMemAddr);
#endif
...
bool SecFimc::destroy()
{
    ...
    ALOGD("SecFimc::destroy()\n");

    /*
    if(mFimcOvlyMode == FIMC_OVLY_DMA_AUTO)
        ALOGD("############# SecFimc::destroy(FIMC_OVLY_DMA_AUTO) called \n");
    else if(mFimcOvlyMode == FIMC_OVLY_NONE_SINGLE_BUF)
        ALOGD("############# SecFimc::destroy(FIMC_OVLY_NONE_SINGLE_BUF) called \n");
    else
        ALOGD("############# SecFimc::destroy(%x) called \n", mFimcOvlyMode);
    */
#ifdef DEBUG_LIB_FIMC
    ALOGD("%s", __func__);
#endif
...
#ifdef DEBUG_LIB_FIMC
    ALOGD("%s:: format= %d, width = %, height= %d, cropW= %d, cropH= %d", __func__, colorFormat, width, height, *cropWidth, *cropHeight);
#endif
                           ...
                           bool forceChange)
{
    ...
    {
        ALOGE("%s:: not supported color format(0x%x)", __func__, colorFormat);
        return false;
    }
