...
bool SecG2d::create(void)
{
    ...
    if (t_create() == false) {
        ALOGE("%s::t_create() fail", __func__);
        goto CREATE_DONE;
    }
...
bool SecG2d::destroy(void)
{
    ...
    if (t_destroy() == false) {
        ALOGE("%s::t_destroy() fail", __func__);
        goto DESTROY_DONE;
    }
                     ...
                     G2dFlag *flag)
{
    ...
    if (m_flagCreate == false) {
        ALOGE("%s::This is not Created fail", __func__);
        goto STRETCH_DONE;
    }
...
{
    ALOGE("%s::This is empty virtual function fail", __func__);
    return false;
}
...
unsigned int SecG2d::t_frameSize(G2dRect * rect)
{
    ...
    switch(rect->color_format)
    {
        ...
        default :
            ALOGE("%s::no matching source colorformat(%d), w(%d), h(%d) fail",
                             ...
                             G2dFlag *flag)
{
    ...
    if (g2d == NULL) {
        ALOGE("%s::createSecG2d() fail", __func__);
        return -1;
    }
