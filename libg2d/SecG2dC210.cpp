...
bool SecG2dC210::t_sync(void)
{
    ...
    if (ioctl(m_fd, FIMG2D_BITBLT_WAIT) < 0) {
        ALOGE("%s::G2D_Sync fail", __func__);
        goto SYNC_FAIL;  
    }
#else
    if (m_pollG2d(&m_poll) == false) {
        ALOGE("%s::m_pollG2d() fail", __func__);
        goto SYNC_FAIL;  
    }
#endif
    return true;
...
bool SecG2dC210::m_doG2d(G2dRect *src, G2dRect *dst, G2dClip *clip, G2dFlag *flag)
{
...
#endif
        ALOGE("%s::FIMG2D_BITBLT_CONFIG fail", __func__);
        return false;
    }
    ...
    if (ioctl(m_fd, FIMG2D_BITBLT_UPDATE, &m_user_region) < 0) {
        ALOGE("%s::FIMG2D_BITBLT_UPDATE fail", __func__);
        return false;
    }

    if (ioctl(m_fd, FIMG2D_BITBLT_CLOSE) < 0) {
        ALOGE("%s::FIMG2D_BITBLT_CLOSE fail", __func__);
        return false;
    }

    // kcoolsw
    ALOGD("#### kcoolsw blit_wait 1");
    //if ((flag->render_mode & RENDER_MODE_NON_BLOCKING) == 0) {
        if (ioctl(m_fd, FIMG2D_BITBLT_WAIT) < 0) {
                ALOGE("%s::FIMG2D_BITBLT_WAIT fail", __func__);
                return false;
        }
    //}
    ALOGD("#### kcoolsw blit_wait 2");
    return true;
}
...
inline bool SecG2dC210::m_pollG2d(struct pollfd *events)
{
    ...
    if (ret < 0) {
        ...
        if (ioctl(m_fd, G2D_RESET) < 0) {
            ALOGE("%s::G2D_RESET fail", __func__);
        }
        */
...
inline bool SecG2dC210::m_cleanG2d(unsigned int virtAddr, unsigned int size)
{
    ...
    if (ioctl(m_fd, FIMG2D_DMA_CACHE_CLEAN, &dma_info) < 0) {
        ALOGE("%s::FIMG2D_DMA_CACHE_CLEAN(%0d, %d) fail", __func__, virtAddr, size);
        return false;
    }
    return true;
}
...
inline bool SecG2dC210::m_flushG2d(unsigned int virtAddr, unsigned int size)
{
    ...
    if (ioctl(m_fd, FIMG2D_DMA_CACHE_FLUSH, &dma_info) < 0) {
        ALOGE("%s::FIMG2D_DMA_CACHE_FLUSH(%d, %d) fail", __func__, virtAddr, size);
        return false;
    }
    return true;
}
...
void SecG2dC210::m_PrintSecG2dArg(G2dRect *src, G2dRect *dst, G2dClip *clip, G2dFlag *flag)
{
    ALOGD("---------------------------------------");
    if (src) {    
        ALOGD("src.color_format : %d", src->color_format);
        ALOGD("src.full_w       : %d", src->full_w);
        ALOGD("src.full_h       : %d", src->full_h);
        ALOGD("src.x            : %d", src->x);
        ALOGD("src.y            : %d", src->y);
        ALOGD("src.w            : %d", src->w);
        ALOGD("src.h            : %d", src->h);
    ...
    if (dst) {    
        ALOGD("dst.color_format : %d", dst->color_format);
        ALOGD("dst.full_w       : %d", dst->full_w);
        ALOGD("dst.full_h       : %d", dst->full_h);
        ALOGD("dst.x            : %d", dst->x);
        ALOGD("dst.y            : %d", dst->y);
        ALOGD("dst.w            : %d", dst->w);
        ALOGD("dst.h            : %d", dst->h);
    ...
    if (clip) {
        ALOGD("clip->l          : %d", clip->l);
        ALOGD("clip->r          : %d", clip->r);
        ALOGD("clip->t          : %d", clip->t);
        ALOGD("clip->b          : %d", clip->b);

    } else
        ALOGD("clip is NULL");

    if (flag) {
        ALOGD("flag.rotate_val  : %d", flag->rotate_val);
        ALOGD("flag.alpha_val   : %d", flag->alpha_val);
    }
    else
        ALOGD("flag is NULL");
...
#else
    ALOGD("===============================================");
    ALOGD("src[%3d, %3d | %10s] -> dst[%3d, %3d | %10s]",
