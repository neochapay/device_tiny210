    ...
    for (int i = m_curSecG2dC110Index; i < NUMBER_G2D_LIST; i++) {
        ...
        if (m_ptrG2dList[i]->flagCreate() == false) {
            if (m_ptrG2dList[i]->create() == false) {
                ALOGE("%s::create(%d) fail", __func__, i);
...
bool SecG2dC110::t_create(void)
{
    ...
    if (m_createG2d() == false) {
        ALOGE("%s::m_createG2d() fail", __func__);

        if (m_destroyG2d() == false)
            ALOGE("%s::m_destroyG2d() fail", __func__);
...
bool SecG2dC110::t_stretch(G2dRect *src, G2dRect *dst, G2dClip *clip, G2dFlag *flag)
{
    ...
    if (src && src->phys_addr == 0) {
        ...
        if (m_srcSize < srcMidRectSize) {
            ALOGE("%s::src size is too big fail", __func__);
            return false;
        }
    }
    ...
    if (m_pollG2d(&m_poll) == false) {
        ALOGE("%s::m_pollG2d 0() fail", __func__);
        return false;
    }
#endif
    ...
    if (src && src->phys_addr == 0) {
        if (t_copyFrame(src, srcMidRect) == false) {
            ALOGE("%s::t_copyFrame() fail", __func__);
            return false;
        }
        ...
        if (m_cleanG2d(srcMidRect->phys_addr, srcMidRectSize) == false) {
            ALOGE("%s::m_cleanG2d() fail", __func__);
            return false;
        }
    ...
    if (dst->phys_addr == 0) {
        if (m_invalidG2d(dstMidRect->phys_addr, dstMidRectSize) == false) {
            ALOGE("%s::m_invalidG2d() fail", __func__);
            return false;
        }
...
#ifdef G2D_NONE_BLOCKING_MODE
        if (m_pollG2d(&m_poll) == false) {
            ALOGE("%s::m_pollG2d 1() fail", __func__);
            return false;
        }
#endif
        ...
        if (m_pollG2d(&m_poll) == false) {
            ALOGE("%s::m_pollG2d 2() fail", __func__);
            return false;
        }
    }
#endif
...
bool SecG2dC110::m_createG2d(void)
{
    ...
    if (m_fd != 0) {
        ALOGE("%s::m_fd(%d) is not 0 fail", __func__, m_fd);
        return false;
    }
...
#endif
    if (m_fd < 0) {
        ALOGE("%s::open(%s) fail(%s)", __func__, SEC_G2D_DEV_NAME, strerror(errno));
        m_fd = 0;
        return false;
    }
...
bool SecG2dC110::m_doG2d(G2dRect *src, G2dRect *dst, G2dFlag *flag)
{
    ...
    if (ioctl(m_fd, G2D_BLIT, &params) < 0) {
        ALOGE("%s::G2D_BLIT fail", __func__);

#if 0
        ALOGE("---------------------------------------");
        if (src) {
            ALOGE("src.color_format : %d", src->color_format);
            ALOGE("src.full_w       : %d", src->full_w);
            ALOGE("src.full_h       : %d", src->full_h);
            ALOGE("src.x            : %d", src->x);
            ALOGE("src.y            : %d", src->y);
            ALOGE("src.w            : %d", src->w);
            ALOGE("src.h            : %d", src->h);
            ALOGE("src.phys_addr    : %x", src->phys_addr);
        }
        else
            ALOGE("flag.color_val   : %d", flag->color_val);

        ALOGE("dst.color_format : %d", dst->color_format);
        ALOGE("dst.full_w       : %d", dst->full_w);
        ALOGE("dst.full_h       : %d", dst->full_h);
        ALOGE("dst.x            : %d", dst->x);
        ALOGE("dst.y            : %d", dst->y);
        ALOGE("dst.w            : %d", dst->w);
        ALOGE("dst.h            : %d", dst->h);
        ALOGE("dst.phys_addr    : %x", dst->phys_addr);

        ALOGE("flag.rotate_val  : %d", flag->rotate_val);
        ALOGE("flag.alpha_val   : %d(%d)", flag->alpha_val);
        ALOGE("flag.color_key_mode  : %d(%d)", flag->color_key_mode, flag->color_key_val);
        ALOGE("---------------------------------------");
#endif
...
inline bool SecG2dC110::m_pollG2d(struct pollfd *events)
{
    ...
    if (ret < 0) {
        ALOGE("%s::poll fail", __func__);
        return false;
    } else if (ret == 0) {
        ALOGE("%s::No data in %d milli secs..", __func__, G2D_POLL_TIME);
        return false;
    }
...
inline bool SecG2dC110::m_cleanG2d(unsigned int physAddr, unsigned int size)
{
    ...
    if (ioctl(m_fd, G2D_DMA_CACHE_CLEAN, &dma_info) < 0) {
        ALOGE("%s::G2D_DMA_CACHE_CLEAN(%d, %d) fail", __func__, physAddr, size);
        return false;
    }
    return true;
}
...
inline bool SecG2dC110::m_invalidG2d(unsigned int physAddr, unsigned int size)
{
    ...
    if (ioctl(m_fd, G2D_DMA_CACHE_INVAL, &dma_info) < 0) {
        ALOGE("%s::G2D_DMA_CACHE_INVAL(%d, %d) fail", __func__, physAddr, size);
        return false;
    }
    return true;
}
...
inline bool SecG2dC110::m_flushG2d(unsigned int physAddr, unsigned int size)
{
    ...
    {
        ALOGE("%s::G2D_DMA_CACHE_FLUSH(%d, %d) fail", __func__, physAddr, size);
        return false;
    }
    return true;
}
...
inline int SecG2dC110::m_colorFormatSecG2d2G2dHw(int color_format)
{
    ...
    default :
        ALOGE("%s::not matched frame format : %d", __func__, color_format);
        break;
    }
    return -1;
}
...
bool SecG2dC110::m_getCalibratedNum(unsigned int *num1, unsigned int *num2)
{
    ...
    if (h == 1) {
        ...
        if (mulNum == 0) {
            ...
            ALOGD("%s::calibration for (%d,%d) is fail", __func__, w, h);
            return false;
        }
        ...
        if (mulNum == 0) {
            // there is no calibration number..
            ALOGE("%s::calibration for (X,1) and (1,X) is fail", __func__);
            return false;
        }
...
#else
    ALOGE("===============================================");
    ALOGE("src[%3d, %3d | %10s] -> dst[%3d, %3d | %10s]",
